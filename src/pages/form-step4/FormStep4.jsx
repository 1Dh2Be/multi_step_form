...
const FormStep4 = () => {
  const { setCurrentStep, isChecked, formData, resetForm } = useForm();

  const planPrice =
    PLAN_PRICE[formData.step2.plan][formData.step2.billingCycle];

  const addonsPrice = Object.keys(formData.step3.addons).reduce(
    (total, key) => {
  ...
  return (
    ...
    >
      <section className="plan__summary">
        <div className="plan__summary--content">
          <section className="current__plan">
            ...
            <span className="curremt__plan--price">{`$${planPrice}/${
              isChecked ? "yr" : "mo"
            }`}</span>
          </section>
          ...
          <section>
            <ul className="addons__list">
              {Object.keys(formData.step3.addons).map(
                (key) =>
                  formData.step3.addons[key] === 1 && (
                    <li className="addon__list" key={key}>
                      ...
                      <span className="addon__price">{`+$${
                        ADDON_PRICE[key][formData.step2.billingCycle]
                      }/${isChecked ? "yr" : "mo"}`}</span>
                    </li>
                  )
              )}
            </ul>
      ...
      <section className="plan__total">
        <h3 className="plan__total--title">{`Total (per ${formData.step2.billingCycle.slice(
          0,
          -2
        )})`}</h3>
        <span className="plan__total--price">{`$${totalPrice}/${
          isChecked ? "yr" : "mo"
        }`}</span>
      </section>
    </FormTemplate>
